(async function () {
  ...
  const detail = await api('/api/investor/' + encodeURIComponent(code));
  if (!detail) return;
  ...
  const verdict = (() => {
    ...
    if (p >= 50) return `${detail.displayName} ate. Up ${p.toFixed(1)}%. Pure cooking. 🔥`;
    if (p >= 5) return `${detail.displayName} grinding — ${p.toFixed(1)}% in the green 📈`;
    if (p > -5) return `${detail.displayName} flat. Neither winning nor losing. Just vibes.`;
    if (p > -20) return `${detail.displayName} down ${Math.abs(p).toFixed(1)}%. It happens.`;
    return `${detail.displayName} cooked. ${Math.abs(p).toFixed(1)}% down. RIP. 💀`;
  })();

  root.innerHTML = `
    <div class="hero">
      <h2>${detail.displayName} <span class="text-muted">(${detail.code})</span></h2>
