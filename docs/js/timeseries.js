...
(function () {
  const { INVESTOR_CODES, classify, splitForSecurity, evenSplit, isRealizingSell, isPricedBuy, amountNok } = window.Ledger;
  ...
  function applyTx(state, tx, attrMap) {
    ...
    const isPriced = isPricedBuy(tx) || isRealizingSell(tx.type);
    if (!isPriced) return;
    for (const { code, weight } of split) {
