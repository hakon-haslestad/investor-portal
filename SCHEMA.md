...
| `Type`           | string  | Legacy field. Currently unused by the engine but kept for sheet compat.     |
| `Mode`           | string  | Legacy field. Currently unused.                                             |
| `Metric`         | string  | Legacy field. Currently unused.                                             |
| `StartDate`      | date    | Competition window start.                                                   |
| `EndDate`        | date    | Competition window end.                                                     |
| `NarrativeJson`  | string  | JSON blob containing the 7-slide narrative copy. See note below.            |
| `CreatedBy`      | string  | Email of creator.                                                           |
| `CreatedAt`      | string  | ISO timestamp.                                                              |

Soft-delete: setting `Id` to empty makes the row invisible to the
portal. The row can stay in the sheet, or be removed manually.

### `NarrativeJson` shape

```json
{
  "subtitle": "Started November 2025. Buy-in 50k. Mode: stock-pick.",
  "setup_teaser": "Each picked their bet …",
  "early_teaser": "First month, no big moves …",
  "pivot_title": "The pivot",
  "pivot_teaser": "Cadeler started running. Some chased.",
  "verdict": ""
}
```

Used to drive the 7-slide presentation mode. Any missing key falls
back to an autogenerated phrase.
